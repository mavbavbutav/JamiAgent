...
- `ApprovalService` backed by SQLAlchemy and connected to orchestrator approval gating
- `ConversationService` for conversation/message persistence
- `AuditService` for DB-backed event logs
- SQLAlchemy models for `conversations`, `messages`, `tool_calls`, `approvals`, and `audit_log`
